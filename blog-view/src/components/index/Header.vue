<template>
  <header ref="header">
    <div class="view">
      ...
      <!-- bg1 -->
      <div v-if="isVideo(defaultSettings.bg1)" class="bg1">
        <video ref="videoBg1" autoplay loop muted v-show="loaded" :src="defaultSettings.bg1" type="video/mp4"
               style="width: 100%; height: 100%; object-fit: cover;"></video>
      </div>
      <div v-else class="bg1" :style="{backgroundImage:'url('+defaultSettings.bg1+')'}"></div>

      <!-- bg2 -->
      <div v-if="isVideo(defaultSettings.bg2)" class="bg2">
        <video autoplay loop muted v-show="loaded" :src="defaultSettings.bg2" type="video/mp4"
               style="width: 100%; height: 100%; object-fit: cover;"></video>
      </div>
      <div v-else class="bg2" :style="{backgroundImage:'url('+defaultSettings.bg2+')'}"></div>

      <!-- bg3 -->
      <div v-if="isVideo(defaultSettings.bg3)" class="bg3">
        <video autoplay loop muted v-show="loaded" :src="defaultSettings.bg3" type="video/mp4"
               style="width: 100%; height: 100%; object-fit: cover;"></video>
      </div>
      <div v-else class="bg3" :style="{backgroundImage:'url('+defaultSettings.bg3+')'}" v-show="loaded"></div>
    </div>
...
export default {
  ...
  mounted() {
     ...
     */
    /**
     * 修改为视频的onloadeddata事件，如果bg1是视频则监听视频加载完成，设置loaded为true
     */
    if (this.isVideo(this.defaultSettings.bg1)) {
      this.$refs.videoBg1.onloadeddata = () => {
        this.loaded = true;
      };
    } else {
